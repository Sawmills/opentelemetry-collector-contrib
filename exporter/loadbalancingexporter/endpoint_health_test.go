...
func TestEndpointHealthUnderPressureReportsActiveSignals(t *testing.T) {
	...
	t.Run("transport quarantine", func(t *testing.T) {
		now := time.Unix(100, 0)
		manager := newEndpointHealthManager(endpointHealthSettings{
			enabled:            true,
			quarantineDuration: 30 * time.Second,
	...
	t.Run("probe unhealthy", func(t *testing.T) {
		now := time.Unix(100, 0)
		manager := newEndpointHealthManager(endpointHealthSettings{
			enabled:            true,
			quarantineDuration: 30 * time.Second,
	...
	t.Run("cached fail open", func(t *testing.T) {
		now := time.Unix(100, 0)
		manager := newEndpointHealthManager(endpointHealthSettings{
			enabled:            true,
			quarantineDuration: 30 * time.Second,
		...
		manager.reconcile([]string{"endpoint-1", "endpoint-2"})
		manager.markFailure("endpoint-1", status.Error(codes.Unavailable, "backend unavailable"))
		manager.markFailure("endpoint-2", context.DeadlineExceeded)

		require.True(t, manager.underPressure())
	})
}

func TestEndpointHealthUnderPressureDoesNotRefreshExpiredQuarantine(t *testing.T) {
	now := time.Unix(100, 0)
	manager := newEndpointHealthManager(endpointHealthSettings{
		enabled:            true,
		quarantineDuration: 30 * time.Second,
	...
	failureReason := state.failureReason
	manager.mu.RUnlock()
	require.True(t, quarantinedUntil.Before(now))
	require.Equal(t, endpointFailureUnavailable, failureReason)
}

func TestEndpointHealthUnderPressureMatchesNoRefreshRoutingForExpiredQuarantines(t *testing.T) {
	now := time.Unix(100, 0)
	manager := newEndpointHealthManager(endpointHealthSettings{
		enabled:               true,
		quarantineDuration:    30 * time.Second,
	...
	eligible := manager.eligibleEndpointsNoRefresh()
	require.Len(t, eligible, 99)
}

func TestEndpointHealthUnderPressureUsesCachedPressureSnapshot(t *testing.T) {
	now := time.Unix(100, 0)
	manager := newEndpointHealthManager(endpointHealthSettings{
		enabled:               true,
		quarantineDuration:    30 * time.Second,
	...
	require.Equal(t, endpointHealthPressureSnapshot{present: 110, eligible: 99, pressured: 11}, manager.pressureSnapshot)
	require.True(t, manager.underPressure())
	...
	refresh := manager.refreshExpiredQuarantines()
	require.NotEmpty(t, refresh.recovered)
	require.Equal(t, endpointHealthPressureSnapshot{present: 110, eligible: 110}, manager.pressureSnapshot)
	require.False(t, manager.underPressure())
}
