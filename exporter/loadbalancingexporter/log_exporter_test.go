...
func TestConsumeLogsCentralQueueBalancesTargetClusterLanesAcrossHealthyBackends(t *testing.T) {
	codec := newQueuePayloadCodec(QueuePayloadCompressionZstd)
	t.Cleanup(func() { require.NoError(t, codec.Close()) })
	const laneCount = 5
	endpoints := centralQueueTargetClusterTestEndpoints()
...
func TestLogsCentralQueueConsumersSendWindowsConcurrently(t *testing.T) {
	...
	require.NoError(t, err)
	second, err := newCentralQueueLogsItem([]byte(route2), simpleLogs(), codec, time.Now())
	require.NoError(t, err)
...
func TestLogsCentralQueueConsumersSerializeSameBackendWindows(t *testing.T) {
	ts, tb := getTelemetryAssets(t)
	cfg := endpoint2Config()
	codec := newQueuePayloadCodec(QueuePayloadCompressionZstd)
	t.Cleanup(func() { require.NoError(t, codec.Close()) })
