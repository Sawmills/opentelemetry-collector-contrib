...
func TestMetricsCentralQueueWindowSkipsInvalidPayload(t *testing.T) {
	ts, tb := getTelemetryAssets(t)
	codec := newQueuePayloadCodec(QueuePayloadCompressionZstd)
	t.Cleanup(func() { require.NoError(t, codec.Close()) })

	endpoint := "endpoint-1:4317"
	var gotServiceNames []string
	p := &metricExporterImp{
		centralCodec: codec,
		loadBalancer: &loadBalancer{
			ring: newHashRing([]string{endpoint}),
			exporters: map[string]*wrappedExporter{endpoint: newWrappedExporter(newMockMetricsExporter(func(_ context.Context, md pmetric.Metrics) error {
				gotServiceNames = serviceNamesFromMetrics(md)
				return nil
			}), endpoint)},
			endpointHealth: newEndpointHealthManager(endpointHealthSettings{}),
		},
	...
	valid, err := newCentralQueueMetricsItem([]byte("lane-a"), metricsWithServiceNames("valid"), codec, time.Now())
	require.NoError(t, err)
	invalid := valid
	invalid.payload = []byte("not-valid-zstd")
	invalid.compressedBytes = len(invalid.payload)

	err = p.consumeCentralQueueMetricWindow(t.Context(), centralQueueWindow{
		routingKey: []byte("lane-a"),
		items:      []centralQueueItem{invalid, valid},
	})
	require.NoError(t, err)
	require.Equal(t, []string{"valid"}, gotServiceNames)
}
...
func TestMetricsCentralQueueReroutesDeadlineExceededEndpoint(t *testing.T) {
	...
	componentFactory := func(_ context.Context, endpoint string) (component.Component, error) {
		return newMockMetricsExporter(func(_ context.Context, md pmetric.Metrics) error {
			...
			delivered.Add(int64(md.DataPointCount()))
			return nil
		}), nil
	}
	lb, err := newLoadBalancer(ts.Logger, cfg, componentFactory, tb)
	...
	require.Eventually(t, func() bool {
		...
		return calls["endpoint-1:4317"] == 1 &&
			...
			delivered.Load() == int64(expectedDelivered) &&
...
func TestMetricsCentralQueueConsumersSendWindowsConcurrently(t *testing.T) {
	...
	first, err := newCentralQueueMetricsItem([]byte(route1), metricsWithServiceNames(route1), codec, time.Now())
	require.NoError(t, err)
	second, err := newCentralQueueMetricsItem([]byte(route2), metricsWithServiceNames(route2), codec, time.Now())
	require.NoError(t, err)
...
func TestMetricsCentralQueueConsumersSerializeSameBackendWindows(t *testing.T) {
	ts, tb := getTelemetryAssets(t)
	cfg := endpoint2Config()
	codec := newQueuePayloadCodec(QueuePayloadCompressionZstd)
	t.Cleanup(func() { require.NoError(t, codec.Close()) })
...
func TestMetricsCentralQueueBackendWaitDoesNotHoldConsumerSlot(t *testing.T) {
	ts, tb := getTelemetryAssets(t)
	cfg := endpoint2Config()
	codec := newQueuePayloadCodec(QueuePayloadCompressionZstd)
	t.Cleanup(func() { require.NoError(t, codec.Close()) })
	...
	first, err := newCentralQueueMetricsItem([]byte(route1), metricsWithServiceNames(route1), codec, time.Now())
	require.NoError(t, err)
	second, err := newCentralQueueMetricsItem([]byte(route1), metricsWithServiceNames(route1), codec, time.Now())
	require.NoError(t, err)
	third, err := newCentralQueueMetricsItem([]byte(route2), metricsWithServiceNames(route2), codec, time.Now())
	require.NoError(t, err)
	require.NoError(t, p.centralQueue.enqueue(first))
	require.NoError(t, p.centralQueue.enqueue(second))
...
func TestMetricsCentralQueueConsumersCapConcurrencyByRoutableBackends(t *testing.T) {
	ts, tb := getTelemetryAssets(t)
	cfg := endpoint2Config()
	codec := newQueuePayloadCodec(QueuePayloadCompressionZstd)
	t.Cleanup(func() { require.NoError(t, codec.Close()) })
	...
	for i := range 4 {
		routingKey := fmt.Sprintf("%s-%d", route, i)
		item, err := newCentralQueueMetricsItem([]byte(routingKey), metricsWithServiceNames(routingKey), codec, time.Now())
		require.NoError(t, err)
		require.NoError(t, p.centralQueue.enqueue(item))
	}
