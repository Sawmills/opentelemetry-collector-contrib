...
* In central queue mode, isolated endpoint-health failures in a large backend set are treated as local routing failures, not global backend pressure. The central queue only throttles drain concurrency for systemic backend pressure, so a single bad backend does not unnecessarily stall draining for a many-worker deployment.
