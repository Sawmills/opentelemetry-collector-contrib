...
func resourceAttrGetter(tCtx TransformContext, key string) (ir.Value, error) {
	val, ok := tCtx.GetResource().Attributes().Get(key)
	if !ok {
...
func resourceAttrSetter(tCtx TransformContext, key string, val ir.Value) error {
	return ctxutil.SetMapValueFromVM(tCtx.GetResource().Attributes(), key, val)
}
