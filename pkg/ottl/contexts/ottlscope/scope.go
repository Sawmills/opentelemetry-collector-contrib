...
func NewParser(
	...
	opts := append([]ottl.Option[TransformContext]{
		ottl.WithVMAttrGetter[TransformContext](scopeAttrGetter),
		ottl.WithVMAttrSetter[TransformContext](scopeAttrSetter),
		ottl.WithVMAttrContextNames[TransformContext]([]string{ctxscope.Name}),
	}, options...)
...
func scopeAttrGetter(tCtx TransformContext, key string) (ir.Value, error) {
	val, ok := tCtx.GetInstrumentationScope().Attributes().Get(key)
	if !ok {
...
func scopeAttrSetter(tCtx TransformContext, key string, val ir.Value) error {
