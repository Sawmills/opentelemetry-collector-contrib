...
func RunWithStackAndContext[K any](stack []ir.Value, p *Program[K], ctx context.Context, tCtx K) (ir.Value, error) {
	if len(p.Code) == 1 {
		...
		case ir.OpAttrEqConstJumpIfFalsePop:
			...
			if attrVal.Type != p.Consts[constIdx].Type || attrVal.Num != p.Consts[constIdx].Num {
				return ir.BoolValue(false), nil
			}
			return ir.BoolValue(true), nil

		case ir.OpAttrNeConstJumpIfFalsePop:
			attrIdx, constIdx := ir.UnpackAttrConst(arg)
			if int(attrIdx) >= len(p.Accessors) {
				return ir.Value{}, ErrInvalidAccessor
			}
		...
		case ir.OpAttrEqConstStringJumpIfFalsePop:
			attrIdx, constIdx := ir.UnpackAttrConst(arg)
			if int(attrIdx) >= len(p.Accessors) {
				return ir.Value{}, ErrInvalidAccessor
			}
			...
			if !stringsEqual(attrVal, p.Consts[constIdx]) {
				return ir.BoolValue(false), nil
			}
			return ir.BoolValue(true), nil

		case ir.OpAttrNeConstStringJumpIfFalsePop:
			attrIdx, constIdx := ir.UnpackAttrConst(arg)
			if int(attrIdx) >= len(p.Accessors) {
				return ir.Value{}, ErrInvalidAccessor
			}
		...
		case ir.OpAttrFastEqConstJumpIfFalsePop:
			keyIdx, constIdx := ir.UnpackAttrConst(arg)
			if p.AttrGetter == nil {
				return ir.Value{}, ErrInvalidAccessor
			}
		...
		case ir.OpAttrFastNeConstJumpIfFalsePop:
			keyIdx, constIdx := ir.UnpackAttrConst(arg)
			if p.AttrGetter == nil {
				return ir.Value{}, ErrInvalidAccessor
			}
		...
		case ir.OpAttrFastEqConstStringJumpIfFalsePop:
			keyIdx, constIdx := ir.UnpackAttrConst(arg)
			if p.AttrGetter == nil {
				return ir.Value{}, ErrInvalidAccessor
			}
		...
		case ir.OpAttrFastNeConstStringJumpIfFalsePop:
			keyIdx, constIdx := ir.UnpackAttrConst(arg)
			if p.AttrGetter == nil {
				return ir.Value{}, ErrInvalidAccessor
			}
