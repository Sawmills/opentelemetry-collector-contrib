...
func splitMetricByAttributes[K any](
	...
	// Track outputs generated by this function instead of treating every prefix match as synthetic.
	var generatedMu sync.Mutex
	generatedMetricNames := make(map[string]int)
	markGeneratedMetric := func(name string) {
		generatedMu.Lock()
		defer generatedMu.Unlock()
		generatedMetricNames[name]++
	}
	consumeGeneratedMetric := func(name string) bool {
		generatedMu.Lock()
		defer generatedMu.Unlock()
		count := generatedMetricNames[name]
		if count == 0 {
			return false
		}
		if count == 1 {
			delete(generatedMetricNames, name)
		} else {
			generatedMetricNames[name] = count - 1
		}
		return true
	}
	...
	return func(_ context.Context, tCtx K) (any, error) {
		...
		if consumeGeneratedMetric(currentMetric.Name()) {
			return nil, nil
		}
		...
		case pmetric.MetricTypeGauge:
			...
			if newDataPoints.Len() > 0 {
				...
				markGeneratedMetric(newMetricName)
