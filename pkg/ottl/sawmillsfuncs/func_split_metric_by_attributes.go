...
func splitMetricByAttributes[K any](
	...
	generatedMetrics := make(map[pmetric.Metric]int)
	markGeneratedMetric := func(metric pmetric.Metric) {
		generatedMu.Lock()
		defer generatedMu.Unlock()
		generatedMetrics[metric]++
	}
	consumeGeneratedMetric := func(metric pmetric.Metric) bool {
		generatedMu.Lock()
		defer generatedMu.Unlock()
		count := generatedMetrics[metric]
		if count == 0 {
			return false
		}
		if count == 1 {
			delete(generatedMetrics, metric)
		} else {
			generatedMetrics[metric] = count - 1
		}
		return true
	}
	...
	return func(_ context.Context, tCtx K) (any, error) {
		...
		if consumeGeneratedMetric(currentMetric) {
			return nil, nil
		}
