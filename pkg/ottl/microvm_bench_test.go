...
const realWorldExpr = `(
	...
	and IsMatch(attributes["service"], "^customer-ml.*") 
	and not (
	...
	and not IsMatch(resource.attributes["ddtags"]["pod_name"], "^customer-ml-worker.*") 
	and not IsMatch(resource.attributes["ddtags"]["pod_name"], "^customer-ml-scheduler.*") 
...
func BenchmarkOTTLInterpreterRealWorld(b *testing.B) {
	...
	pathValues := map[string]any{
		...
		"attributes[service]":                                  "customer-ml-foo",
