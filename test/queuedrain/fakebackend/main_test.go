...
func TestBuildLoadgenRequestSetsServiceTenantAndBatch(t *testing.T) {
	req := buildLoadgenRequest("payload", 3, "customer-local-queue-drain", "customer")
	...
	if got := resourceAttrs[0].GetValue().GetStringValue(); got != "customer-local-queue-drain" {
		t.Fatalf("service.name = %q, want customer-local-queue-drain", got)
	}
	...
	if got := recordAttrs[0].GetValue().GetStringValue(); got != "customer" {
		t.Fatalf("tenant = %q, want customer", got)
	}
}
