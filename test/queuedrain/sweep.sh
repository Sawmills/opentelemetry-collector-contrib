...
ARTIFACT_ROOT="${ROOT_DIR}/artifacts/queue-drain/sweep-$(date -u +%Y%m%dT%H%M%SZ)"
...
usage() {
  cat <<'EOF'
Run the queue-drain request-size sweep.

Usage:
  test/queuedrain/sweep.sh [options] [-- run.sh options]
...
Examples:
  test/queuedrain/sweep.sh -- --payload-profile repeated --payload-size-bytes 1048576
EOF
}
