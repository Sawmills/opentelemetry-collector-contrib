...
"""
    ...
    def backend_metrics(records: int, pinned: bool = False) -> str:
        return f"""
queuedrain_backend_log_records_total {records}
queuedrain_backend_received_log_records_total {records}
queuedrain_backend_failures_total {1 if pinned else 0}
"""

    @staticmethod
    def tally_metrics(received: int, accepted: int, failed: int) -> str:
        return f"""
queuedrain_tally_received_log_records_total {received}
queuedrain_tally_accepted_log_records_total {accepted}
queuedrain_tally_failed_log_records_total {failed}
"""
