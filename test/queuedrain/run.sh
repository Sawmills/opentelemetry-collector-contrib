...
HARNESS_DIR="${ROOT_DIR}/test/queuedrain"
...
BATCH_SIZE="100"
LOAD_SIZE_MB="0"
LOG_BODY="customer-local-queue-drain-log"
...
scrape_loop() {
  ...
  while (( SECONDS < end )); do
    ...
    scrape_pod_metrics "${NAMESPACE}" "app=queue-drain-lb" 8888 "lb-metrics" "${tick_id}" "${out_dir}"
    scrape_pod_metrics "${NAMESPACE}" "app=queue-drain-backend" 13133 "backend-metrics" "${tick_id}" "${out_dir}"
    scrape_service_metrics "${NAMESPACE}" "queue-drain-tally" 13134 "tally-metrics" "${tick_id}" "${out_dir}"
    tick=$((tick + 1))
    sleep "${interval}"
  done
}
...
capture_k8s() {
  ...
  kubectl get pods -n "${NAMESPACE}" -l app=queue-drain-lb -o json > "${out_dir}/pods-${suffix}.json" || true
...
run_phase() {
  ...
  kubectl rollout status -n "${NAMESPACE}" statefulset/queue-drain-backend --timeout=180s || return 1
  kubectl rollout status -n "${NAMESPACE}" deployment/queue-drain-tally --timeout=180s || return 1
  kubectl rollout status -n "${NAMESPACE}" deployment/queue-drain-lb --timeout=180s || return 1
  ...
  kubectl set env -n "${NAMESPACE}" statefulset/queue-drain-backend \
    SLOW_AFTER=1ns \
  ...
  if kubectl rollout status -n "${NAMESPACE}" statefulset/queue-drain-backend --timeout="${rollout_timeout}s" > "${out_dir}/rollout-status.txt" 2>&1; then
    echo "backend rollout completed"
  else
    echo "backend rollout did not complete; see ${out_dir}/rollout-status.txt"
  fi

  kubectl wait -n "${NAMESPACE}" --for=condition=complete job/queue-drain-loadgen --timeout="$((LOAD_DURATION_SECONDS + SETTLE_SECONDS))s" \
    > "${out_dir}/loadgen-wait.txt" 2>&1 || true
  kubectl logs -n "${NAMESPACE}" job/queue-drain-loadgen > "${out_dir}/loadgen.log" 2>&1 || true
  ...
  scrape_pod_metrics "${NAMESPACE}" "app=queue-drain-lb" 8888 "lb-metrics" "999999" "${out_dir}"
  scrape_pod_metrics "${NAMESPACE}" "app=queue-drain-backend" 13133 "backend-metrics" "999999" "${out_dir}"
  scrape_service_metrics "${NAMESPACE}" "queue-drain-tally" 13134 "tally-metrics" "999999" "${out_dir}"
